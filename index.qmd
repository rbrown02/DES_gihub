---
title: "Discrete Event Simulation Outputs"
format:
    html:
        toc: true
...
The median wait for registration across `{python} g.number_of_runs` runs was `{python} "{:.1f}".format(df_trial_results['Mean Queue Time Reg'].median())` minutes.

The median wait for booking a test was `{python} "{:.1f}".format(df_trial_results['Mean Queue Time Book Test'].median())` minutes.

The median wait for callers to have their call answered was `{python} "{:.1f}".format(df_trial_results['Mean Queue Time Call'].median())` minutes.

The median utilisation for a receptionist was `{python} "{:.1f}".format(df_trial_results['Receptionist Utilisation - Percentage'].median())`%.

### GP Stats

The median wait for a GP across `{python} g.number_of_runs` runs was `{python} "{:.1f}".format(df_trial_results['Mean Queue Time GP'].median())` minutes.

The median utilisation for a GP across was `{python} "{:.1f}".format(df_trial_results['GP Utilisation - Percentage'].median())`%.

# Results Table

The results of our DES model are as follows:
